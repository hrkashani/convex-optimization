...
We also provide the following helper functions for plotting and other utility:
    ...
    - visualize_data(): Generates a plot of y, and s * a_true.
        Call plt.plot() after calling the function to make the plot visible.
...
def visualize_estimate(a):
    ...
    plt.plot(times,np.convolve(s,a),label = 'Estimated',linestyle = '-.')
    plt.ylabel(r's * $\hat{a}$')
    
    plt.subplot(313)
    plt.plot(times[:2000],a_true,label = '$True$',linewidth = 3)
    plt.plot(times[:2000],a,label = '$Estimated$',linestyle = '-.')
    plt.ylabel(r'$\hat{a}$')
    plt.legend()
    plt.xlabel('t')
